...
This says that ~foo~ and ~bar~ should listed in the ~buildInputs~
...
package named ~zlib~. This is a bit confusing, but remember that the
keys here are Cabal packages, while the values are Nix packages.
