...
mkCabbages() {
  ...
  if [ "${#BUILDTOOLS}" -gt 0 ]; then
    ...
    for bt in "${BUILDTOOLS[@]}"; do
      cabal get "$bt"
      if ! [ $? -eq 0 ]; then
        echo "Build tool $bt is not available from Hackage"
      else
...
# Check /nix/store for a buildplan for this versioned
# package. If not found, run dryDependencies, otherwise use the old
...
freezeIfNeeded() {
  ...
  for d in $(ls -d /nix/store/*-haskell-"$NAME" 2> /dev/null; ls -d /nix/store/*-haskell-"$NAME".4552 2> /dev/null); do
    if [ -f "$d"/.cabbageCache/buildplan ]; then
      cp "$d"/.cabbageCache/buildplan ./buildplan
      chmod u+w buildplan
      return 0
    fi
  done
