...
# Remove globally installed packages from consideration. First line of
# input should be ghc-pkg list --global --simple-output
filterOutGlobals2() {
  local AWK
  read -r -d '' AWK<<'EOF'
BEGIN {
...
freezeCabbagePatch() {
  ...
  if [ $? -eq 1 ]; then
    ...
    ((echo "$GLOBALS"; cat cabal.config.bak) | filterOutGlobals) > cabal.config
  fi
  rm cabal.config.bak
}
#+END_SRC
