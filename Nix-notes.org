...
$(if [ "${#TOOLS[@]}" -gt 0 ]; then
    ...
    for t in "${TOOLS[@]}"; do
      ...
      if ! [ buildTool = "" ]; then
        ...
        echo "WARNING: Couldn't add shell.nix dependency on $t"
      fi
    done
    echo "    };"
  fi)
...
** TODO Resolve build-tool to package name mapping
A problem is that packages sometimes list a build-tool executable that is different than the name of the hackage package that provides that build tool. We can't depend on those things, of course.
