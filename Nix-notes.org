...
once in the shell, run ~sh "$setup"~.
...
getMyFullName() {
  ...
  { (cat "$CABAL" | grep "^[Nn]ame:" | sed 's/^[Nn]ame:[[:space:]]*\(.*\)$/\1/');
    (cat "$CABAL" | grep "^[Vv]ersion:" | sed 's/^[Vv]ersion:[[:space:]]*\(.*\)$/\1/'); } \
  | tr '\n' '-' | sed 's/-$//'
}
...
EOF
    ...
    LINKSANDBOX="mkdir -p $(pwd)/.cabal-sandbox/lib/$1/ && ln -sFf \${p}/.cabal-sandbox/lib/$1/* $(pwd)/.cabal-sandbox/lib/ && ln -sFf \${pkg.outPath}/.cabal-sandbox/$1-packages.conf.d/*.conf "$(pwd)"/.cabal-sandbox/$1-packages.conf.d/";
...
on another add-source dependency.
