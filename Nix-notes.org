...
SETUPEOF
  ...
  builder = builtins.toFile "builder.sh" ''
    ...
    \${cabalTmp} --builddir=\$out/dist --bindir=\$out/bin --libexecdir=\$out/libexec --libdir=\$out/.cabal-sandbox/lib --with-gcc=\$CC configure \$(echo \$NIX_LDFLAGS | awk -e '{ for(i=1;i <= NF; i++) { if(match(\$(i), /^-L/)) printf("--extra-lib-dirs=%s ", substr(\$(i),3)); } }')
    echo "Building..."
...
$(if [ "${#TOOLS[@]}" -gt 0 ]; then
    ...
    local t
    for t in "${TOOLS[@]}"; do
      local buildTool=$(find .cabbages -name "$t-[[:digit:].]*" -depth 1)
      if ! [ "$buildTool" = "" ]; then
...
** TODO Packages with build-tool /and/ library
=cpphs= is a trouble maker here. It gets built as a build-tool, which makes module /not/ require a =frozenCabbages= argument, but it needs to be built as a library as well, and, in that case, it /does/ need a =frozenCabbages=  argument. When we build the build tool executable from the downstream =shell.nix=, it is self-contained, but when we build the library from downstream =default.nix=, it should be linked with all the other libraries.
