...
# Print the array of all buildable build-tools (ie not hsc2hs or ghc)
# used to build this package and all of its dependencies.
allBuildableBuildTools() {
  local TOOLS=($(allBuildToolsAux | uniq))

  # We don't build hsc2hs or ghc
  local special
  for special in hsc2hs ghc; do
    local i=$(findIndex "$special" TOOLS[@])
    if [ "$i" -gt -1 ]; then
...
callCabbages() {
  ...
  for ((i = 0; i < ${#NAMES[@]}; ++i)); do
    local TOOLSARR=($(cd .cabbages/${PKGS[$i]} && cabalBuildTools))
    local TOOLS=""
    if [ ${#TOOLSARR[@]} -gt 0 ]; then
      for special in hsc2hs ghc; do
        local j=$(findIndex "$special" TOOLSARR[@])
        if [ "$j" -gt -1 ]; then
          unset TOOLSARR[$j]
        fi
      done
  ...
  builder = builtins.toFile "builder.sh" ''
    ...
    if [ -d "\$src" ]; then
      ...
      if [ -f \$src/cabal.config.bak ]; then
        ...
        cp "\$src/cabal.config.bak" "\$out/.cabbageCache/cabal.config.bak"
      fi
    else
      tar xf "\$src" --strip=1
    fi
...
freezeIfNeeded() {
  ...
  for d in $(ls -d /nix/store/*-haskell-"$NAME" 2> /dev/null; ls -d /nix/store/*-haskell-"$NAME".4552 2> /dev/null); do
    if [ -f "$d"/.cabbageCache/cabal.config.bak ]; then
      cp "$d"/.cabbageCache/cabal.config.bak ./cabal.config.bak
...
** TODO Pull out "special" build-tool support
Just search for "special" to see each place we filter out un-buildable
build tools (i.e. those that come with GHC).
** TODO Build tools that need build tools
=c2hs=, for example, needs both =happy= and =alex=. The way cabbage
currently works, those tools end up in =c2hs='s =.cabbage= directory,
but then =c2hs='s =default.nix= needs them as arguments. So what we
have to do is call the build tool packages in the top-level
=shell.nix=, and pass them to =c2hs= when we call it from the
top-level =shell.nix=.
** TODO Unpack frozenCabbages
The next big step is to not pass the entire set of packages to each
dependency, but only the ones that are needed.
