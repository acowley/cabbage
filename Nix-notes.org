...
getAddSource() {
  local CWD=$(pwd)
  (cd $1 && cabal sdist --verbose=0 --output-directory="$CWD"/.cabbages/$(getMyFullName))
}
#+END_SRC
...
getPackageDBPath() {
  if [ -f cabal.sandbox.config ]; then
    echo $(cabal sandbox hc-pkg list | grep ".conf.d" | tail -n 1 | sed 's/.*\/\(.*\)-packages.conf.d.*/\1/')
    return 0
  else
    return 1
  fi
...
mkCabbage() {
  ...
  for ((i=0; i < ${#DEPS[@]}; ++i)); do
    ...
    DEPNAMES[$i]=$(echo "${DEPS[$i]}" | sed 's/\(.*\)-[[:digit:]].*/\1/')
  done
