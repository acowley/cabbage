...
guarded behind "if os(windows)" and "if os(solaris)".
...
# os(solaris)" conditional. This is a very fragile test!
removeWindowsBlocks() {
  local AWK
  read -r -d '' AWK<<'EOF'
...
warnExtraLibraries() {
  ...
  if [ "${#LIBS[@]}" -gt 0 ]; then
    ...
    echo "See the cabbage documentation for how to do this with "
    echo "a 'systemDeps' section in a cabbage.config file."
...
** Build-tools
#+NAME: allBuildTools
#+BEGIN_SRC sh :tangle cabbage
cabalBuildTools() {
  cat ./*.cabal | sed 's/--.*$//' | removeWindowsBlocks | grep -i "build-tools" | \
...
allBuildTools() {
  local TOOLS=($(allBuildToolsAux | sed 's/,/\
/' | uniq))
  echo "${TOOLS[@]}"
}
#+END_SRC
...
# Freezes the cabal file in the current directory. If the argument to
# this function is equal to false, then this is not a downstream
# cabbage and its build plan should be intersected with a downstream
# package's build plan. If the argument is true, then this /is/ a
# downstream package.
prepCabbage() {
  if [ -f cabbage.config ]; then
...
This may just be a matter of running ~cabal sandbox init
--sandbox=../../.cabal-sandbx~ in each upstream package's cabbage
directory before running ~cabal freeze~.
...
What we want is to have unique derivations for the build-tools, but
not intersect those build plans with the package we are building.
...
- We filter out the [Bb]uild-[Tt]ools sections of /all/ .cabal files,
  including upstream packages.
- We get source for the tools in the .cabbages directory as usual.
- We include callPackage calls for each build tool from the downstream
  shell.nix.
- We include those derivations in the ~haskellBuildTools~ array.
- Ensure this works with named package installations.
